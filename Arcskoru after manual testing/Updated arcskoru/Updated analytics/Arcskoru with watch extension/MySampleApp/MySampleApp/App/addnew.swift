...
class addnew: UIViewController, UITableViewDataSource, UITableViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        unit.addTarget(self, action: #selector(self.textchange(sender:)), for: .valueChanged)
        value.addTarget(self, action: #selector(self.textchange(sender:)), for: .valueChanged)
        ...
        if(edit == 1){
            ...
            currentfield = "1232"
            selected_field = currentfield
            selected_year = df.string(from: date)
        }
