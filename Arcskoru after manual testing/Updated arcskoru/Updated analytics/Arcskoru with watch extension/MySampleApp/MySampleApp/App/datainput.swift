...
class datainput: UIViewController, UITableViewDataSource,UITableViewDelegate,UITabBarDelegate,UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func editcurrentmeter(_ sender:UIButton){
        self.edit = 1
        if((self.currentarr["CreditDescription"] as! String).lowercased() == "water"){
            self.cat = 1
        }else{
            if let dict = currentmeter["fuel_type"] as? NSDictionary{
                if((dict["kind"] as! String).lowercased() == "electricity"){
                    self.cat = 0
                    self.type = 0
                }else if((dict["kind"] as! String).lowercased() == "fuel"){
                    self.cat = 2
                    self.type = 0
                }
            }
        ...
        print(sender.tag - 10)
        currentmeter = (self.meters.object(at: sender.tag - 10) as! NSDictionary).mutableCopy() as! NSMutableDictionary
        var n = ""
        for i in t{
            var v = i as! NSArray
            if let dict = currentmeter["fuel_type"] as? NSDictionary{
                if let typ = dict["type"] as? String{
                    for st in v{
                        let str = st as! String
                    if(str == typ){
                        n = typ
                        break
                    }
                    ...
                    print( dict["subtype"] as? String)
                }
            }
        }
        print("Before",currentmeter["fuel_type"])
        if(n == ""){
            currentmeter["fuel_type"] = [
                "id": 25,
        ...
        print("After",currentmeter["fuel_type"])
    ...
    func delcurrentmeter(_ sender:UIButton){
        currentmeter = (self.meters.object(at: sender.tag - 10) as! NSDictionary).mutableCopy() as! NSMutableDictionary
        let alertController = UIAlertController(title: "Delete meter \(currentmeter["name"] as! String)", message: "Do you want to delete the meter and associated data with the meter", preferredStyle: .alert)
...
//        print(selectedreading)
        id = dict["id"] as! Int
        selectedreading = NSArray.init(array: self.arrayofreadings["\(id)"] as! NSArray)
        
        for d in selectedreading{
            let dict = d as! NSDictionary
            print("Before",dict["reading"])
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.section == 0){
           ...
           let view = GraphView()
                ...
                if(self.meterdates.count>0){
                    ...
                    if(datesdict.count > 0){
                        ...
                        if(dict["end_date"] != nil){
                            var date2 =  dict["end_date"] as! Date
                            let calendar = Calendar.current
                            date2 = (calendar as NSCalendar).date(byAdding: .day, value: -1, to: date2, options: [])!
            ...
            cell.heading.text =  str
                ...
                actualstring.append(tempostring.mutableCopy() as! NSAttributedString)
                tempostring.deleteCharacters(in: NSMakeRange(0, tempostring.length))
                if((d["kind"] as! String).lowercased() == "electricity" && d["type"] as! String != "Generated onsite - solar" && d["type"] as! String != "Purchased from Grid"){
        ...
        tableView.frame.origin.y = 0//tableview.frame.origin.y + tableview.frame.size.height
        var dict = (currentfeeds.object(at: indexPath.row) as! NSDictionary).mutableCopy() as! NSMutableDictionary
        cell.textLabel?.text = dict["verb"] as? String
        cell.textLabel?.adjustsFontSizeToFitWidth = true
        cell.selectionStyle = UITableViewCellSelectionStyle.none
    ...
    func navigate(){
        ...
        DispatchQueue.main.async(execute: {
            self.currentmeter = NSMutableDictionary()
            self.meters = NSMutableArray()
            self.datesdict = NSMutableDictionary()
            self.currentfeeds = NSArray()
            self.readingsdict = NSMutableDictionary()
            self.selectedreading = NSArray()
            self.meterdates = NSMutableArray()
            self.download_requests = [URLSession]()
            self.uploadsdata = NSArray()
            self.arrayofreadings = NSMutableDictionary()
            self.entirereadingsarr = NSMutableArray()
            self.currentmetersdict = NSMutableDictionary()
            self.startdatearr = NSMutableArray()
            self.enddatearr = NSMutableArray()
            self.graphPoints = NSArray()
            self.samplegraphdata = [Int]()
            self.tempgraphdata = [Int]()
            self.readingsarr = NSMutableArray()
            self.selectedreading = NSArray()
    ...
    func getmeters(_ subscription_key:String, leedid: Int, kind: String){
        let url = URL.init(string:String(format: "%@assets/LEED:%d/meters/?kind=%@&page_size=50",domain_url, leedid,kind))
        ////print(url?.absoluteURL)
        ...
        self.task = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
            ...
            if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode == 401 {           // check for http errors
                ...
                do {
                    ...
                    DispatchQueue.main.async(execute: {
                    //self.getmeterdata()
                        self.graphPoints = NSArray.init(array: self.meters)
        ...
        if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode == 401 {           // check for http errors
            DispatchQueue.main.async(execute: {
                ...
                do {
                    ...
                    var t = NSMutableArray.init(array: arr)
                    var  sortDescriptor = NSSortDescriptor.init(key: "end_date", ascending: true)
                    t.sort(using: NSArray.init(object: sortDescriptor) as! [NSSortDescriptor])
                    arr = NSArray.init(array: t as NSArray)
                    print(self.arrayofreadings,self.entirereadingsarr)
                    
                    for i in 0..<arr.count {
                        var dict = (arr.object(at: i) as! NSDictionary).mutableCopy() as! NSMutableDictionary
                        if(dict["reading"]as! Float > 0){
                            let l = dict["reading"]as! Float
                            guard !(l.isNaN || l.isInfinite) else {
                                self.tempgraphdata.append(0)
                                break
                            }
    ...
    var currentmeter = NSMutableDictionary()
    /*
     // MARK: - Navigation
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if(segue.identifier == "gotoreadings"){
            ...
            vc.type = self.type
            vc.cat = cat
            vc.s = currentarr["CreditDescription"] as! String
            ...
            print(currentmeter["native_unit"] as! String)
                print(currentmeter ["fuel_type"] as! NSDictionary)
                /*if(type == 0){
