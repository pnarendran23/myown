...
class manageparking: UIViewController,UITableViewDelegate, UITableViewDataSource, UIAlertViewDelegate, UITextViewDelegate, UITabBarDelegate, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == 0){
            ...
            cell.textLabel?.numberOfLines = 3
            cell.detailTextLabel?.textColor = UIColor.black
            if(temp_dict["updated_at"] != nil && (temp_dict["updated_at"] as? String)?.characters.count > 0){
                let dateFormatter: DateFormatter = DateFormatter()
                dateFormatter.dateFormat = credentials().milli_secs
                if(dateFormatter.date(from: temp_dict["updated_at"] as! String) != nil){
                let date = dateFormatter.date(from: temp_dict["updated_at"] as! String)!
            ...
            cell.textLabel?.numberOfLines = 3
            print(temp_dict)
            if(temp_dict["IsLovRecert"] != nil){
                
                if(temp_dict["IsLovRecert"] as? Bool == true){
                    cell.detailTextLabel?.text = temp_dict["PrevCertProdId"] as? String
                }else{
                ...
                if((temp_dict["IsLovRecert"] as? Bool)! == false){
                    cell.detailTextLabel?.text = "No"
                }else{
                    if(temp_dict["PrevCertProdId"] != nil && (temp_dict["PrevCertProdId"] as? String)?.characters.count > 0){
                        cell.detailTextLabel?.text = temp_dict["PrevCertProdId"] as? String
                    }else{
                        cell.detailTextLabel?.text = ""
                    }
            ...
            }else if(temp_dict["IsLovRecert"] != nil && (temp_dict["IsLovRecert"] is Int)){
                if(temp_dict["IsLovRecert"] as! Int == 0){
                    cell.detailTextLabel?.text = "No"
                }else{
                    if(temp_dict["PrevCertProdId"] != nil && (temp_dict["PrevCertProdId"] as? String)?.characters.count > 0){
                        cell.detailTextLabel?.text = temp_dict["PrevCertProdId"] as? String
                ...
                if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode != 200 {           // check for http errors
                    ...
                    do {
                        ...
                        DispatchQueue.main.async(execute: {
                            ...
                            self.spinner.isHidden = true
                            self.view.isUserInteractionEnabled = true
                            self.dict = NSMutableDictionary.init(dictionary: jsontemp_dictionary)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "gotoname"){
            ...
            let dest = segue.destination as! enableandfillViewController
            dest.context = currentcontext
            dest.data_dict = temp_dict
        }
    }
