...
class manageacity: UIViewController, UITableViewDataSource, UITableViewDelegate, UITabBarDelegate, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(t == "cities"){
            if(indexPath.row == 0){
                ...
                cell.segmentedctrl.insertSegment(withTitle: "IP", at: 0, animated: false)
                cell.segmentedctrl.insertSegment(withTitle: "SI", at: 1, animated: false)
                ...
                if let s = data_dict["unitType"] as? String{
                    if(s == "IP"){
                        cell.segmentedctrl.selectedSegmentIndex = 0
                    }else if(s == "SI"){
                ...
                cell.selectionStyle = .none
                if(data_dict["leed_id"] == nil || data_dict["leed_id"] is NSNull){
                    cell.detailTextLabel?.text = ""
                }else{
                    cell.detailTextLabel?.text = "\(data_dict["leed_id"] as! Int)"
        ...
        if(indexPath.row == 0){
            ...
            cell.segmentedctrl.removeAllSegments()
            cell.textLabel?.font = UIFont.init(name: "OpenSans", size: 15)
            cell.segmentedctrl.insertSegment(withTitle: "LEED for \(t )", at: 0, animated: false)
            cell.segmentedctrl.insertSegment(withTitle: "Other", at: 1, animated: false)
            cell.segmentedctrl.insertSegment(withTitle: "None", at: 2, animated: true)
            cell.segmentedctrl.addTarget(self, action: #selector(self.segmentchange(_:)), for: UIControlEvents.valueChanged)
            cell.segmentedctrl.selectedSegmentIndex = 0
            if let s = data_dict["rating_system"] as? String{
            if(s == "none"){
                cell.segmentedctrl.selectedSegmentIndex = 2
            }else if(s == "LEED-CM"){
                cell.segmentedctrl.selectedSegmentIndex = 0
            }else if(s == "other"){
                cell.segmentedctrl.selectedSegmentIndex = 1
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
         if(segue.identifier == "gotoselectedvalue"){
            let v = segue.destination as! selectvalue
            v.data_dict = data_dict
        if(currentcontext == "Year Founded"){
