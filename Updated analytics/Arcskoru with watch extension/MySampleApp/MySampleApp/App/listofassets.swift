...
class listofassets: UIViewController, UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate, WCSessionDelegate {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if(segue.identifier == "filterproj"){
            ...
            var va = v.viewControllers[0] as! gridviewcontroller
    ...
    */
    ...
    func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        return 0.166 * UIScreen.mainScreen().bounds.size.height;
        }
        return 0.166 * UIScreen.mainScreen().bounds.size.width;
        }
        return 50
    }
        ...
        if(tableView == filtertable){
            let cell = tableView.cellForRowAtIndexPath(indexPath)!
                cell.accessoryType = UITableViewCellAccessoryType.None
                        tobefiltered.replaceObjectAtIndex(indexPath.section, withObject: "")
        }
        ...
        tempdict = countries["divisions"]![currentbuilding["country"] as! String] as! [String:AnyObject]
         ...
         break
            ...
            //self.getperformancedata(c.subscription_key, leedid: currentleedid, date: datee)
                    ...
                    NSUserDefaults.standardUserDefaults().setInteger(indexPath.section, forKey: "currentrow")
            ...
            dispatch_async(dispatch_get_main_queue(), {
               ...
               showalert("Please make the payment for this project to proceed", title: "Error", action: "OK")
                    ...
                    dispatch_async(dispatch_get_main_queue(),{
                        self.spinner.hidden = false
                        self.view.userInteractionEnabled = false
                        NSUserDefaults.standardUserDefaults().setInteger(currentbuilding["leed_id"] as! Int, forKey: "leed_id")
                        NSUserDefaults.standardUserDefaults().setObject(currentbuilding["key"], forKey: "key")
...
//        return "Portfolios"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if(tableView == filtertable){
            ...
            if(tobefiltered.objectAtIndex(indexPath.section) as? String != ""){
        ...
        tempostring.addAttribute(NSFontAttributeName, value: UIFont.init(name: "OpenSans-Semibold", size: 0.05 * UIScreen.mainScreen().bounds.size.width)!, range: NSMakeRange(0, tempostring.length))
        ...
        tempostring.addAttribute(NSFontAttributeName, value: UIFont.init(name: "OpenSans", size: 0.03 * UIScreen.mainScreen().bounds.size.width)!, range: NSMakeRange(0, tempostring.length))
        actualstring.appendAttributedString(tempostring.mutableCopy() as! NSAttributedString)
        tempostring.deleteCharactersInRange(NSMakeRange(0, tempostring.length))
        tempostring = NSMutableAttributedString(string:" ")
        actualstring.appendAttributedString(tempostring.mutableCopy() as! NSAttributedString)
        ...
        self.task = session.dataTaskWithRequest(request) { data, response, error in
            ...
            if let httpStatus = response as? NSHTTPURLResponse where httpStatus.statusCode != 200 {           // check for http errors
                ...
                do {
                    ...
                    if(self.tobefiltered != "all" && self.page == 1){
