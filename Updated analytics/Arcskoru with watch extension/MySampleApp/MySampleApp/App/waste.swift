...
class waste: UIViewController,UITabBarDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UITableViewDelegate, UITableViewDataSource, MFMailComposeViewControllerDelegate {
   ...
   /* override func supportedInterfaceOrientations() -> UIInterfaceOrientationMask {
    ...
    override func viewDidLoad() {
        ...
        tableview.frame.size.height = 0.06 * UIScreen.mainScreen().bounds.size.height
            ...
            if let httpStatus = response as? NSHTTPURLResponse where httpStatus.statusCode == 401 {           // check for http errors
                ...
                do {
                    ...
                    dispatch_async(dispatch_get_main_queue(), {
                        ...
                        if(self.currentfeeds.count > 0){
                        self.feedstable.frame = CGRect(x:0,y:self.feedstable.frame.origin.y,width:self.feedstable.frame.width, height: CGFloat(self.currentfeeds.count) * (0.197 * UIScreen.mainScreen().bounds.size.height))
                        }else{
                        self.feedstable.frame = CGRect(x:0,y:self.feedstable.frame.origin.y,width:self.feedstable.frame.width, height: CGFloat(2.5) * (0.197 * UIScreen.mainScreen().bounds.size.height))
                        }
                        var i = CGFloat(0)
                        if((self.currentarr["CreditDescription"] as! String).lowercaseString == "waste"){
                            i = i + ((0.054 * UIScreen.mainScreen().bounds.size.height) * CGFloat(self.currentfeeds.count))
                                }else{
                                    i = i + (0.197 * UIScreen.mainScreen().bounds.size.height)
                                    i = i + ((0.107 * UIScreen.mainScreen().bounds.size.height) * CGFloat(self.currentfeeds.count))
                                }
                        if(self.currentfeeds.count > 0){
                        i = i + CGFloat(self.currentfeeds.count + 2)
                        i = i + ((0.035 * UIScreen.mainScreen().bounds.size.height) * CGFloat(self.currentfeeds.count + 2))
                        i = i + ((0.017 * UIScreen.mainScreen().bounds.size.height) * CGFloat(self.currentfeeds.count + 2))
                        }else{
                        i = i + CGFloat(2)
                        i = i + ((0.035 * UIScreen.mainScreen().bounds.size.height) * CGFloat(2))
                        i = i + ((0.017 * UIScreen.mainScreen().bounds.size.height) * CGFloat(2))
                        }
                        ...
                        print("max Y is",s)
                        self.sview.contentSize = CGSizeMake(UIScreen.mainScreen().bounds.size.width, s)
    ...
    func navigate(){
        self.view.bringSubviewToFront(self.feedstable)
        ...
        self.task = s.dataTaskWithRequest(request) { data, response, error in
            ...
            if let httpStatus = response as? NSHTTPURLResponse where httpStatus.statusCode != 200 {           // check for http errors
                ...
                do {
                    ...
                    if(UIDevice.currentDevice().orientation == .Portrait){
                            self.sview.contentSize = CGSizeMake(UIScreen.mainScreen().bounds.size.width, 1.6 * UIScreen.mainScreen().bounds.size.height)
                    }else{
                ...
                self.feedstable.frame = CGRect(x:0,y:view.frame.origin.y+view.frame.size.height,width:self.feedstable.frame.width, height: self.feedstable.layer.frame.size.height)
                    ...
                    self.sview.contentSize = CGSizeMake(UIScreen.mainScreen().bounds.size.width,totaly)
            ...
            if let httpStatus = response as? NSHTTPURLResponse where httpStatus.statusCode != 200 {           // check for http errors
                ...
                do {
                    ...
                    dispatch_async(dispatch_get_main_queue(),{
                        ...
                        NSUserDefaults.standardUserDefaults().setObject(d, forKey: "data")
                        ...
                        self.cc.frame.size.width = 0.42 * UIScreen.mainScreen().bounds.size.height
    ...
    @IBAction func assignthemember(sender: UIButton) {
        if(statusupdate == 1){
            self.view.userInteractionEnabled = false
            self.spinner.hidden = false
