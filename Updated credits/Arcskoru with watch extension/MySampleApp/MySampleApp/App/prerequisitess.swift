            ...
            if let httpStatus = response as? NSHTTPURLResponse where httpStatus.statusCode == 401 {           // check for http errors
                ...
                do {
                    ...
                    dispatch_async(dispatch_get_main_queue(), {
                        ...
                        self.feedstable.frame.size.height = 1.5 * (CGFloat(self.currentfeeds.count * 3) * (0.118 * h))
                        
                        self.sview.contentSize = CGSize(width: UIScreen.mainScreen().bounds.size.width ,height:  (self.affirmationview1.frame.size.height + self.feedstable.frame.size.height))
