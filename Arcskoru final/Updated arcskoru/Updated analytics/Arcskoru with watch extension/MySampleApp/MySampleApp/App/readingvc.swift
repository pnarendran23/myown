    ...
    override func viewDidLoad() {
        ...
        if(addreading == 1){
            ...
            var newenddate = Calendar.current.date(byAdding: .month, value: 1, to: newstartdate!)
                newenddate = Calendar.current.date(byAdding: .day, value: 0, to: newenddate!)
            ...
            dateFormat.dateFormat = "MMM dd,yyyy"
                let calendar = Calendar.current
            newenddate = (calendar as NSCalendar).date(byAdding: .day, value: 0, to: newenddate!, options: [])
            date2 = newenddate!
            str = dateFormat.string(from: newenddate!)
            ...
            let calendar = Calendar.current
            let twoDaysAgo = (calendar as NSCalendar).date(byAdding: .day, value: -1, to: date2, options: [])
    ...
    func showalert(_ index: Int, title : String, value : String){
        DispatchQueue.main.async(execute: {
            ...
            let otherAction = UIAlertAction(title: "Add", style: UIAlertActionStyle.default, handler:{ (UIAlertAction)in
                ...
                self.iterate = 0
                if(self.startdate.text?.characters.count > 0 && self.enddate.text?.characters.count > 0 && self.reading.text?.characters.count > 0){
    ...
    */
    ...
    @IBAction func updateit(_ sender: AnyObject) {
        ...
        ////print(url?.absoluteURL)
            ...
            let calendar = Calendar.current
            dated2 = (calendar as NSCalendar).date(byAdding: .day, value: 1, to: dated2, options: [])!
        ...
        request.addValue("application/json", forHTTPHeaderField:"Content-type" )
        request.addValue(String(format:"Bearer %@",token), forHTTPHeaderField:"Authorization" )
        var httpbody = String(format: "{\"start_date\":\"%@\",\"end_date\":\"%@\",\"reading\":%ld}",startdatestr,enddatestr,Int(reading.text!)!)
        ...
        task.resume()
            ...
            request.addValue("application/json", forHTTPHeaderField:"Content-type" )
            request.addValue(String(format:"Bearer %@",token), forHTTPHeaderField:"Authorization" )
            let httpbody = String(format: "{\"start_date\":\"%@\",\"end_date\":\"%@\",\"reading\":%ld}",startdatestr,enddatestr,Int(reading.text!)!)
