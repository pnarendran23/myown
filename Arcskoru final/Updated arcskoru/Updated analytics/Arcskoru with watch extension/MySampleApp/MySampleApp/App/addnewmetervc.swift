...
class addnewmetervc: UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        unitarr = ["kWh","MWh","MBtu","kBtu","GJ"]
        unitsarr = [["gal","kGal","MGal","cf","ccf","kcf","mcf","l","cu m","gal(UK)","kGal(UK)","kGal(UK)","MGal(UK)","cGal (UK)","cGal (US)","Kcm"],["kWh","MWh","therms","kBtu","MBtu","GJ","cu m","l","gal","kGal","MGal","cf","ccf","kcf","mcf"]]
        otherfuelsarr = [["kWh","MWh","therms","kBtu","MBtu","GJ","Lbs","KLbs","MLbs","kg"],["kWh","MWh","therms","kBtu","MBtu","GJ"],
            ...
            ["kWh","MWh","therms","kBtu","MBtu","GJ","ton hours"],
            ["kWh","MWh","therms","kBtu","MBtu","GJ","ton hours"],
            ["kWh","MWh","therms","kBtu","MBtu","GJ","cu m","l","gal","kGal","MGal","cf","ccf","kcf","mcf"],
            ["kWh","MWh","therms","kBtu","MBtu","GJ","cu m","l","gal","kGal","MGal","cf","ccf","kcf","mcf"],
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.async(execute: {
            if(self.cat == 2){
                self.currentunit = self.otherfuelsarr[0] as! NSArray
            }else if(self.cat == 0){
                self.currentunit = self.unitarr as! NSArray
                if(self.edit == 1){
                if let dict = self.currentmeter as? NSDictionary{
                            ...
                            if(self.type == 0 && self.s.lowercased() == "energy"){
                                ...
                                if let dict = self.currentmeter as? NSDictionary{
                                    ...
                                    if((dict["kind"] as! String).lowercased() == "electricity"){
                                        ...
                                        self.currentunit = self.otherfuelsarr as! NSArray
                                        self.currentmetertype = self.metertype[2] as! NSArray
                                        if(self.edit == 1){
                                            self.currentmetertype = self.othersarr as! NSArray
                                            for i in 0 ..< self.currentmetertype.count {
                                                var dict = self.currentmetertype[i] as! NSDictionary
                                                if(dict["type"] as! String == self.currentmeter["type"] as! String){
                                                    self.currenttype = self.currentmeter["type"] as! String
                                                    self.typepicker.selectRow(i, inComponent: 0, animated: true)
                                                    self.currentunit = self.otherfuelsarr[i] as! NSArray
                                                    break
                                                }
                                            }
                                        }
    ...
    @IBAction func typedone(_ sender: AnyObject) {
        ...
        currentunit = unitarr as! NSArray
                    if(self.edit == 1){
                    if let dict = self.currentmeter as? NSDictionary{
                        if((dict["kind"] as! String).lowercased() == "electricity"){
                            
                        }else if((dict["kind"] as! String).lowercased() == "fuel"){
                            self.currentunit = self.otherfuelsarr[typepicker.selectedRow(inComponent: 0)] as! NSArray
                        }}}
                }else{
                 currentunit = unitsarr[0] as! NSArray
