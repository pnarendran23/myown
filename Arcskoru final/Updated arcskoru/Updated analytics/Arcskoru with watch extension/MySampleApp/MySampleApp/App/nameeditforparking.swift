...
class nameeditforparking: UIViewController, UITextFieldDelegate, UINavigationControllerDelegate, UITabBarDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        var str = (textField.text)?.appending(string)
        if(string != numberFiltered){
            shakeit()
        }
        
        if(((str?.characters.count)! > 2 || str == "0") && self.currenttitle == "How many levels in your parking structure?"){
            shakeit()
        }
        if(self.currenttitle == "How many levels in your parking structure?"){
        return (string == numberFiltered && (str?.characters.count)! <= 2 && str != "0")
        }
