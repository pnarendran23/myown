...
class nameeditforcity: UIViewController, UITextFieldDelegate, UITabBarDelegate, UINavigationControllerDelegate {
    ...
    func textChanged(_ sender : UITextField){
        ...
        if(self.currenttitle == "Year Founded"){
    ...
    @IBAction func save(_ sender: Any) {
        DispatchQueue.main.async(execute: {
            ...
            if(self.currenttitle == "Name"){
                ...
                self.data_dict["managEntityAdd2"] = self.name
            }else if(self.currenttitle == "Managing entity City"){
            ...
            if(self.currenttitle == "Area"){
                ...
                if(a! <= 19305){
                    ...
                    self.maketoast("Please enter value greater than 0", type: "error")
                }else{
                    DispatchQueue.main.async(execute: {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if(context.text == "Name" || context.text == "Managing entity Name" || context.text == "Managing entity Address (line1)" || context.text == "Managing entity Address (line2)" || context.text == "Managing entity City"){
            return true
        }
        ...
        var c = txtfld.text
        if(context.text == "Year Founded"){
            let textString = (textField.text! as NSString).replacingCharacters(in: range, with: string)
            let aSet = NSCharacterSet(charactersIn:"0123456789").inverted
            let compSepByCharInSet = string.components(separatedBy: aSet)
            let numberFiltered = compSepByCharInSet.joined(separator: "")
            print(numberFiltered)
            return (string == numberFiltered && textString.characters.count <= 4)
        }
