    ...
    func proceedwithapp(){
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: {
            ...
            if(response != nil){
                ...
                if (statusCode == 200) {
                    do {
                        if var jsonResult = (try JSONSerialization.jsonObject(with: data!, options: []) as! NSDictionary).mutableCopy() as? NSMutableDictionary{
                            ...
                            var dict = (jsonResult["iOS"] as! NSDictionary).mutableCopy() as! NSMutableDictionary
                            
                            let str = dict["forceToUpdate"] as! Bool
                            let currentVersion = dict["version"] as! String
                ...
                if (statusCode == 200) {
                    do {
                        if let jsonResult = try JSONSerialization.jsonObject(with: data!, options: []) as? NSDictionary{
                            ...
                            /*    jsonResult = jsonResult["scores"] as! NSMutableDictionary as NSDictionary
                             ...
                             dict["wastescore"] = self.prefs.integer(forKey: "wastescore") 
                             dict["transportscore"] = self.prefs.integer(forKey:"transportscore")
                             ...
                             dict["name"] = self.prefs.object(forKey: "name") as! String
                             dict["key"] = self.prefs.object(forKey: "key") as! String
                             dict["leed_id"] = self.leedid
                ...
                if (statusCode == 200) {
                    do {
                        if var jsonResult = try JSONSerialization.jsonObject(with: data!, options: []) as? NSDictionary{
                            //print("COunt is ",jsonResult)
                            jsonResult = (jsonResult["scores"]  as! NSDictionary).mutableCopy() as! NSMutableDictionary
                            if(jsonResult["energy"] is NSNull){
                                UserDefaults.standard.set(0, forKey: "energyscore")
                            }else{
                                let energy = jsonResult["energy"] as! Int
                            ...
                            DispatchQueue.main.sync {
                                ...
                                if(WCSession.isSupported()){
                                    ...
                                    self.prefs.set(self.leedid, forKey: "leed_id")
                                    self.prefs.set(self.key, forKey: "key")
                                    WCSession.default().sendMessage(dict as! [String : Any], replyHandler: ({
                                    (reply) -> Void in
                                    ...
                                    do{
                                        try WCSession.default().updateApplicationContext(dict as! [String : Any])
                                    }catch{
