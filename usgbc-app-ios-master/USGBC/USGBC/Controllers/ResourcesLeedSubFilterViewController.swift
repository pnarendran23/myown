//
//  ResourcesLeedSubFilterViewController.swift
//  USGBC
//
...
protocol ResourcesLeedSubFilterDelegate: class {
    func userDidSelectedSubFilter(filter: ResourcesLeedFilter, changed: Bool, selectedFilter: String,typearray : [String], formatarray : [String], ratingarray :[String], versionarray : [String], accessarray : [String], languagearray : [String])
}
class ResourcesLeedSubFilterViewController: UIViewController {
...
extension ResourcesLeedSubFilterViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        print("Selected \(filter.subFilters[indexPath.row]) \(filter.subFilters.first(where: { $0.value == "2780"})) \(filter.name)")
        //typearray : [String], formatarray : [String], ratingarray : [String], accessarray : [String], languagearray : [String]
        var currentword = String()
            if(filter.subFilters.first(where: { $0.value == filter.subFilters[indexPath.row].value}) != nil){
                    var dict = filter.subFilters.first(where: { $0.value == filter.subFilters[indexPath.row].value}) as! ResourcesLeedSubFilter
