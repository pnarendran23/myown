//
//  ResourcesCredentialingSubFilterViewController.swift
//  USGBC
//
...
protocol ResourcesCredentialingSubFilterDelegate: class {
    func userDidSelectedSubFilter(filter: ResourcesCredentialingFilter, changed: Bool, selectedFilter: String,typearray : [String], formatarray : [String], ratingarray :[String], versionarray : [String], accessarray : [String], languagearray : [String])
}

class ResourcesCredentialingSubFilterViewController: UIViewController {
    var filterString: [String] = []
    weak var delegate: ResourcesCredentialingSubFilterDelegate?
    fileprivate var filterChanged = false
    var filter: ResourcesCredentialingFilter!
...
extension ResourcesCredentialingSubFilterViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(filter.subFilters.first(where: { $0.value == filter.subFilters[indexPath.row].value}) != nil){
            var dict = filter.subFilters.first(where: { $0.value == filter.subFilters[indexPath.row].value}) as! ResourcesCredentialingSubFilter
