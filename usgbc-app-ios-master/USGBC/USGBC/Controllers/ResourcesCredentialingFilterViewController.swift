//
//  ResourcesCredentialingFilterViewController.swift
//  USGBC
//
...
protocol ResourcesCredentialingFilterDelegate: class {
...
class ResourcesCredentialingFilterViewController: UIViewController {
    ...
    weak var delegate: ResourcesCredentialingFilterDelegate?
    ...
    var language: String = ""
    var selectedFilter = ""
    var filter = "7456+5048+2016/all/all/all/all/all"
    var totalCount = 0
 ...
 let parameter = Payloads().makePayloadForResources(typearray: typearray, formatarray: formatarray, ratingarray: ratingarray, versionarray: versionarray, accessarray: accessarray, languagearray: languagearray, currentcategory : "credentialing")
    ...
    @IBAction func handleClearFilter(_ sender: Any) {
            ...
            self.typearray = ["Candidate Handbooks","Certificates","Study guides"]
            self.formatarray = []
            self.ratingarray = []
            self.versionarray = []
            self.accessarray = []
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ResourcesCredentialingSubFilterViewController" {
            if let viewController = segue.destination as? ResourcesCredentialingSubFilterViewController {
                viewController.filter = sender as! ResourcesCredentialingFilter
...
extension ResourcesCredentialingFilterViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "ResourcesCredentialingSubFilterViewController", sender: filters[indexPath.row])
    }
}

extension ResourcesCredentialingFilterViewController: ResourcesCredentialingSubFilterDelegate {
    func userDidSelectedSubFilter(filter: ResourcesCredentialingFilter, changed: Bool,  selectedFilter filterString: String, typearray : [String], formatarray : [String], ratingarray :[String], versionarray : [String], accessarray : [String], languagearray : [String]) {
        if(changed){
            ...
            print("ResourcesCredentialingSubFilterDelegate")
            print(filterString)
            filterChanged = true
