...
- (void) deviceOrientationDidChangeNotification:(NSNotification *)note
{
    ...
    spinner.hidden = NO;
    self.hidden = YES;
    rotatetimer =[NSTimer scheduledTimerWithTimeInterval:(0.5) target:self selector:@selector(rotate:) userInfo:nil repeats:NO];
...
-(void)rotate:(NSNotification *)note{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if(resizee ==1){
            ...
            ww=width1;
            hh=height1;
            if(device.orientation==UIDeviceOrientationPortrait || device.orientation==UIDeviceOrientationLandscapeLeft || device.orientation==UIDeviceOrientationLandscapeRight || device.orientation == UIDeviceOrientationPortraitUpsideDown){ //|| device.orientation==UIDeviceOrientationPortraitUpsideDown){
                
                self.hidden=YES;
                starter=[NSTimer scheduledTimerWithTimeInterval:(0.2) target:self selector:@selector(avoidflick) userInfo:nil repeats:NO];
                ...
                if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
                    ...
                    if(UIDeviceOrientationIsPortrait([UIDevice currentDevice].orientation)){
                        if([lastorientation isEqualToString:@"landscape"] || lastorientation.length==0){
                            self.hidden=YES;
                            starter=[NSTimer scheduledTimerWithTimeInterval:(0.4) target:self selector:@selector(avoidflick) userInfo:nil repeats:NO];
                            lastorientation=@"portrait";
                            self.frame=CGRectMake(0, 0.15*hh, ww, ww);
                        }
                    }
                    else{
                        if([lastorientation isEqualToString:@"portrait"] || lastorientation.length==0){
                            self.hidden=YES;
                            starter=[NSTimer scheduledTimerWithTimeInterval:(0.4) target:self selector:@selector(avoidflick) userInfo:nil repeats:NO];
                    ...
                    if(width1<height1){
                        ...
                        self.frame=CGRectMake(0.2*width1, 0, hh, hh);
                    }
                    if(UIDeviceOrientationIsPortrait([UIDevice currentDevice].orientation)){
                        if([lastorientation isEqualToString:@"landscape"] || lastorientation.length==0){
                            self.hidden=YES;
                        ...
                        if([lastorientation isEqualToString:@"portrait"] || lastorientation.length==0){
                            self.hidden=YES;
                            starter=[NSTimer scheduledTimerWithTimeInterval:(0.4) target:self selector:@selector(avoidflick) userInfo:nil repeats:NO];
                            self.frame=CGRectMake(0.2*ww, 0, hh, hh);
                            lastorientation=@"landscape";
                        }
                    }
                }
...
- (void)tick:(CADisplayLink *)sender
{
    ...
    stepscore.string=[NSString stringWithFormat:@"%d",(int)basevalue];
    
    
    
    startupp=[NSTimer scheduledTimerWithTimeInterval:(2+(0.05*duration)) target:self selector:@selector(addUntitled1Animation) userInfo:nil repeats:NO];
    
    
    if(initiall==YES){
        //        t2=[NSTimer scheduledTimerWithTimeInterval:6.8 target:self selector:@selector(flip) userInfo:nil repeats:NO];
        timer2=[NSTimer scheduledTimerWithTimeInterval:(7.75+(0.05*duration)) target:self selector:@selector(flip) userInfo:nil repeats:NO];
    }
    else{
    ...
    if(initiall==YES){
        //
        timer2=[NSTimer scheduledTimerWithTimeInterval:(15.4+(0.05*duration)) target:self selector:@selector(sum1) userInfo:nil repeats:NO];
    }
    else{
    ...
    if(initiall==YES){
        //
        timer2=[NSTimer scheduledTimerWithTimeInterval:(21.7+(0.2*duration)) target:self selector:@selector(sum2) userInfo:nil repeats:NO];
    }else{
    ...
    if(initiall==YES){
        ...
        timer2=[NSTimer scheduledTimerWithTimeInterval:(28+(0.2*duration)) target:self selector:@selector(sum3) userInfo:nil repeats:NO];
    }else{
    ...
    if(initiall==YES){
        ...
        timer2=[NSTimer scheduledTimerWithTimeInterval:(34+(0.2*duration)) target:self selector:@selector(sum4) userInfo:nil repeats:NO];
    }
    else{
    ...
    if(initiall==YES){
        ...
        timer2=[NSTimer scheduledTimerWithTimeInterval:(39.5+(0.2*duration)) target:self selector:@selector(sum5) userInfo:nil repeats:NO];
    }else{
...
-(void)repeatme{
    ...
    if(initiall==YES){
        ...
        timer2=[NSTimer scheduledTimerWithTimeInterval:(5.5+(0.05*duration)) target:self selector:@selector(flip) userInfo:nil repeats:NO];
    }
...
-(void)adjustlinewidth{
    ...
    if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation))
    {
        ...
        /*   oval.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         oval2.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         oval3.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         oval4.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         oval5.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         humanstarting.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         transportstarting.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         wastestarting.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         waterstarting.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         energystarting.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         humanfill.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         transportfill.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         wastefill.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         waterfill.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         energyfill.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         energystartline.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         waterstartline.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         wastestartline.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         transportstartline.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         humanstartline.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         humanback.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         transportback.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         wasteback.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         waterback.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         energyback.lineWidth=(self.frame.size.width/14)*(self.bounds.size.width/width1);
         text.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         text2.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         text3.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         text4.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         text5.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         humanscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         transportscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         wastescore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         waterscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         energyscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         humanmaxscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         transportmaxscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         wastemaxscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         watermaxscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         energymaxscore.fontSize=(self.frame.size.width/14)*(self.bounds.size.width/width1)/2;
         maxscore.fontSize = 0.52*((self.frame.size.width)*(self.bounds.size.width/width1));
         stepscore.fontSize=0.52*((self.frame.size.width)*(self.bounds.size.width/width1));*/
...
- (void)setupLayers{
    ...
    //  //NSLog(@"URL IS ACTUALLY %@",[NSString stringWithFormat:@"%@%@%@",leedarray[0],leedarray[1],leedarray[2]]);
    
    @try {
        //jsonData = [NSData dataWithContentsOfURL:blogURL];
        
        NSDictionary *dataDictionary = [NSKeyedUnarchiver unarchiveObjectWithData:[[NSUserDefaults standardUserDefaults] objectForKey:@"performance_data"]];
        NSDictionary *key = [dataDictionary objectForKey:@"scores"];
        NSLog(@"Data %@",key);
        if([key objectForKey:@"energy"]== [NSNull null])
        {
            energyvalue=[@"0.00" floatValue];
        }
        else
        {
            energyvalue=[[key objectForKey:@"energy"] floatValue];
        }
        
        if([key objectForKey:@"water"]== [NSNull null])
        {
            watervalue=[@"0.00" floatValue];
        }
        else
        {
            watervalue=[[key objectForKey:@"water"] floatValue];
        }
        
        if([key objectForKey:@"waste"]== [NSNull null])
        {
            wastevalue=[@"0.00" floatValue];
        }
        else
        {
            wastevalue=[[key objectForKey:@"waste"] floatValue];
        }
        
        if([key objectForKey:@"transport"]== [NSNull null])
        {
            transportvalue=[@"0.00" floatValue];
        }
        else
        {
            transportvalue=[[key objectForKey:@"transport"] floatValue];
        }
        
        if([key objectForKey:@"human_experience"]== [NSNull null])
        {
            humanvalue=[@"0.00" floatValue];
        }
        else
        {
            humanvalue=[[key objectForKey:@"human_experience"] floatValue];
        }
        
        if([key objectForKey:@"base"]== [NSNull null])
        {
            basevalue=[@"0.00" floatValue];
        }
        else
        {
            basevalue=[[key objectForKey:@"base"] floatValue];
        }
        ...
        if([key objectForKey:@"energy"]== [NSNull null])
        {
            menergyvalue=[@"0.00" floatValue];
        }
        else
        {
            menergyvalue=[[key objectForKey:@"energy"] floatValue];
        }
        
        if([key objectForKey:@"water"]== [NSNull null])
        {
            mwatervalue=[@"0.00" floatValue];
        }
        else
        {
            mwatervalue=[[key objectForKey:@"water"] floatValue];
        }
        ...
        {
            mwastevalue=[[key objectForKey:@"waste"] floatValue];
        }
        
        if([key objectForKey:@"transport"]== [NSNull null])
        {
            mtransportvalue=[@"0.00" floatValue];
        }
        else
        {
            mtransportvalue=[[key objectForKey:@"transport"] floatValue];
        }
        
        if([key objectForKey:@"human_experience"]== [NSNull null])
        {
            mhumanvalue=[@"0.00" floatValue];
        }
        else
        {
            mhumanvalue=[[key objectForKey:@"human_experience"] floatValue];
        }
        self.hidden=NO;
        ...
        self.layerss[@"dynamic_plaque"] = dynamic_plaque;
        {
            ...
            self.layerss[@"leed_plaque"] = leed_plaque;
            {
                ...
                self.layerss[@"transportgroup"] = transportgroup;
                {
                    ...
                    transportmaxscore.string          = [NSString stringWithFormat:@"%d",(int)mtransportvalue];
                    ...
                    if(transportvalue!=mtransportvalue){
                ...
                self.layerss[@"wastegroup"] = wastegroup;
                {
                    ...
                    wastemaxscore.string          = [NSString stringWithFormat:@"%d",(int)mwastevalue];
                ...
                self.layerss[@"humangroup"] = humangroup;
                {
                    ...
                    humanmaxscore.string          = [NSString stringWithFormat:@"%d",(int)mhumanvalue];
                ...
                self.layerss[@"watergroup"] = watergroup;
                {
                    ...
                    watermaxscore.string          = [NSString stringWithFormat:@"%d",(int)mwatervalue];
                ...
                self.layerss[@"energygroup"] = energygroup;
                {
                    ...
                    energymaxscore.string          = [NSString stringWithFormat:@"%d",(int)menergyvalue];
            ...
            self.layerss[@"plaque_coin"] = plaque_coin;
            {
                ...
                self.layerss[@"booms"] = booms;
                {
                    ...
                    if(transportvalue==0 && wastevalue==0 && watervalue==0 && energyvalue==0){
                        humanboom.strokeStart=0;
                        humanboom.strokeEnd=1;
                        
                    }
                    else if(humanvalue==0 && wastevalue==0 && watervalue==0 && energyvalue==0){
                        transportboom.strokeStart=0;
                        transportboom.strokeEnd=1;
                    }
                    else if(humanvalue==0 && transportvalue==0 && watervalue==0 && energyvalue==0){
                        wasteboom.strokeEnd=1;
                        wasteboom.strokeStart=0;
                    }
                    else if(humanvalue==0 && wastevalue==0 && transportvalue==0 && energyvalue==0){
                        waterboom.strokeStart=0;
                        waterboom.strokeEnd=1;
                    }
                    else if(humanvalue==0 && wastevalue==0 && watervalue==0 && transportvalue==0){
                        ...
                        if(humanvalue>0){
                            humanboom.strokeStart=0;
                            humanboom.strokeEnd=humanboom.strokeStart+(humanvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                        ...
                        if(transportvalue>0){
                            ...
                            NSLog(@"Pie %.1f",transportvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                            
                            transportboom.strokeEnd=transportboom.strokeStart+(transportvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                            //                        transportboom.strokeEnd=transportvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue);
                        }
                        if(wastevalue>0){
                            if(humanvalue==0 && transportvalue==0){
                                wasteboom.strokeStart=0;
                            }else if(humanvalue>0 && transportvalue==0){
                                wasteboom.strokeStart=humanboom.strokeEnd;
                            }
                            else if(humanvalue>0 && transportvalue>0 && wastevalue>0 && watervalue==0){
                                
                            }
                            wasteboom.strokeEnd=wasteboom.strokeStart+(wastevalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                        }
                        if(watervalue>0){
                            if(humanvalue==0 && transportvalue==0 && wastevalue==0){
                                waterboom.strokeStart=0;
                            }else{
                                if(wastevalue==0 && transportvalue>0 && humanvalue>0){
                                    waterboom.strokeStart=transportboom.strokeEnd;
                                }
                                else if(wastevalue==0 && transportvalue==0 && humanvalue>0){
                                    waterboom.strokeStart=waterboom.strokeEnd;
                                }
                                
                            }
                            waterboom.strokeEnd=waterboom.strokeStart+(watervalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                        }
                        if(energyvalue>0){
                            if(humanvalue==0 && transportvalue==0 && wastevalue==0 && watervalue==0){
                                energyboom.strokeStart=0;
                            }
                            else if(humanvalue>0 && transportvalue>0 && wastevalue>0 && watervalue==0){
                            ...
                            energyboom.strokeEnd=energyboom.strokeStart+(energyvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                        }
                        
                        if(energyvalue>0 && watervalue>0 && wastevalue>0 && transportvalue>0 && humanvalue>0){
                            humanboom.strokeStart=0;
                            humanboom.strokeEnd=humanboom.strokeStart+(humanvalue/(humanvalue+transportvalue+wastevalue+watervalue+energyvalue));
                    ...
                    human=humanvalue/mhumanvalue;
                    transport=transportvalue/mtransportvalue;
                    waste=wastevalue/mwastevalue;
                    water=watervalue/mwatervalue;
                    energy=energyvalue/menergyvalue;
                    float totales=(human+transport+waste+water+energy);
                    humanboom.strokeStart=0;
                    humanboom.strokeEnd=humanboom.strokeStart+(human/totales);
                    transportboom.strokeStart=humanboom.strokeEnd;
                    transportboom.strokeEnd=transportboom.strokeStart+(transport/totales);
                    wasteboom.strokeStart=transportboom.strokeEnd;
                    wasteboom.strokeEnd=wasteboom.strokeStart+(waste/totales);
                    waterboom.strokeStart=wasteboom.strokeEnd;
                    waterboom.strokeEnd=waterboom.strokeStart+(water/totales);
                    energyboom.strokeStart=waterboom.strokeEnd;
                    energyboom.strokeEnd=energyboom.strokeStart+(energy/totales);
                    NSLog(@"totall %f %f %f %f %f %f",human,transport,waste,water,energy,totales);
                ...
                totalls=(int)(energyvalue+basevalue+watervalue+wastevalue+transportvalue+humanvalue);
                NSLog(@"%f %f %f %f %f",(humanvalue/mhumanvalue),(transportvalue/mtransportvalue),(wastevalue/mwastevalue),(watervalue/mwatervalue),(energyvalue/menergyvalue));
                platinum.hidden=YES;
                gold.hidden=YES;
                silver.hidden=YES;
                certified.hidden=YES;
                ...
                maxscore.string          = [NSString stringWithFormat:@"%d",(int)(energyvalue+basevalue+watervalue+wastevalue+transportvalue+humanvalue)];
...
-(void)suspendit{
    ...
    t.string=[NSString stringWithFormat:@"%d",(int)basevalue];
    initiall=YES;
...
-(void)sum2{
    ...
    actualtxt=0;
    tmp=0;
    if(nu1>0.25*(int)mtransportvalue){
...
- (UIBezierPath*)energystartingPathWithBounds:(CGRect)bound{
    if(energyvalue>0 && energyvalue!=menergyvalue){
        float f=(180*(energyvalue/menergyvalue));
        f=f+(2*(180-f));
    ...
    else if(energyvalue==menergyvalue){
...
- (UIBezierPath*)energystartinggPathWithBounds:(CGRect)bound{
    if(energyvalue>0 && energyvalue!=menergyvalue){
        float f=(180*((energyvalue-0.2)/menergyvalue));
        f=f+(2*(180-f));
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
        UIBezierPath*  energystartingPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(0, 0) radius:CGRectGetWidth(bound)/2 startAngle:-90 * M_PI/180 endAngle:-f * M_PI/180 clockwise:YES];
        CGAffineTransform pathTransform = CGAffineTransformMakeTranslation(CGRectGetMidX(bound), CGRectGetMidY(bound));;
...
- (UIBezierPath*)waterstartingPathWithBounds:(CGRect)bound{
    if(watervalue>0 && watervalue!=mwatervalue){
        float f=(180*(watervalue/mwatervalue));
        f=f+(2*(180-f));
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
    ...
    else if (mwatervalue==watervalue){
...
- (UIBezierPath*)waterstartinggPathWithBounds:(CGRect)bound{
    if(watervalue>0 && watervalue!=mwatervalue){
        float f=(180*((watervalue-0.2)/mwatervalue));
        f=f+(2*(180-f));
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
        UIBezierPath*  waterstartingPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(0, 0) radius:CGRectGetWidth(bound)/2 startAngle:-90 * M_PI/180 endAngle:-f * M_PI/180 clockwise:YES];
        CGAffineTransform pathTransform = CGAffineTransformMakeTranslation(CGRectGetMidX(bound), CGRectGetMidY(bound));;
...
- (UIBezierPath*)wastestartinggPathWithBounds:(CGRect)bound{
    if(wastevalue>0 && wastevalue!=mwastevalue){
        float f=(180*((wastevalue-0.05)/mwastevalue));
        f=f+(2*(180-f));
        NSLog(@"Score %f",f);
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
    ...
    else if (mwastevalue==wastevalue){
...
- (UIBezierPath*)wastestartingPathWithBounds:(CGRect)bound{
    ...
    if(wastevalue>0 && wastevalue!=mwastevalue){
        float f=(180*((wastevalue)/mwastevalue));
        f=f+(2*(180-f));
        NSLog(@"Arrow %f",f);
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
        UIBezierPath*  wastestartingPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(0, 0) radius:CGRectGetWidth(bound)/2 startAngle:-90 * M_PI/180 endAngle:-f * M_PI/180 clockwise:YES];
...
- (UIBezierPath*)transportstartingPathWithBounds:(CGRect)bound{
    if(transportvalue>0 && transportvalue!=mtransportvalue){
        NSLog(@"Actual %f",bound.size.width);
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
        float f=(180*(transportvalue/mtransportvalue));
        f=f+(2*(180-f));
    ...
    }else if (mtransportvalue==transportvalue){
...
- (UIBezierPath*)transportstartinggPathWithBounds:(CGRect)bound{
    if(transportvalue>0 && transportvalue!=mtransportvalue){
        float f=(180*((transportvalue-0.2)/mtransportvalue));
        f=f+(2*(180-f));
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
        UIBezierPath*  transportstartingPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(0, 0) radius:CGRectGetWidth(bound)/2 startAngle:-90 * M_PI/180 endAngle:-f * M_PI/180 clockwise:YES];
        CGAffineTransform pathTransform = CGAffineTransformMakeTranslation(CGRectGetMidX(bound), CGRectGetMidY(bound));;
...
- (UIBezierPath*)humanstartingPathWithBounds:(CGRect)bound{
    if(humanvalue>0 && humanvalue<mhumanvalue){
        float f=(180*(humanvalue/mhumanvalue));
        f=f+(2*(180-f));
        if (CGRectEqualToRect(bound, CGRectZero)) return nil;
    ...
    else if(humanvalue==mhumanvalue){
