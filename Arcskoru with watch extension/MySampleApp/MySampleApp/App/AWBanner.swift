...
class AWBannerView: UIView {
  ...
  override init(frame: CGRect) {
    ...
    if(UIDevice.currentDevice().orientation == UIDeviceOrientation.LandscapeLeft || UIDevice.currentDevice().orientation == UIDeviceOrientation.LandscapeRight){
        height = 0.058 * UIScreen.mainScreen().bounds.size.height
    }else{
        height = 0.058 * UIScreen.mainScreen().bounds.size.height
    }
    self.setup()
  ...
  func setup() {
    ...
    self.notificationLabel               = UILabel(frame: CGRect(x: 0.0, y: 0.0, width: self.frame.size.width, height: height))
    self.notificationLabel.font          = notificationLabel.font.fontWithSize(15.0)
...
public class AWBanner {
  ...
  public static func showWithDuration(duration: NSTimeInterval, delay: NSTimeInterval, message: String, backgroundColor: UIColor, textColor: UIColor, originY y: CGFloat = originY) {
    ...
    self.notificationView.notificationLabel.font = UIFont.init(name: "OpenSans", size: 15)
    ...
    UIView.animateWithDuration(0.5, delay: delay, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.0, options: .CurveLinear, animations: {
      ...
      self.notificationView.frame = CGRect(x: 0.0, y: originY, width: AWBannerProperties.width, height: height) }, completion: nil)
