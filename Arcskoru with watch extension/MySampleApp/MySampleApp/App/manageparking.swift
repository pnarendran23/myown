...
class manageparking: UIViewController,UITableViewDelegate, UITableViewDataSource, UIAlertViewDelegate, UITextViewDelegate, UITabBarDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if(indexPath.row == 0){
            ...
            cell.textLabel?.numberOfLines = 3
            cell.detailTextLabel?.textColor = UIColor.blackColor()
            if(dict["leed_id"] != nil && (dict["leed_id"] as? String)?.characters.count > 0){
                cell.detailTextLabel?.text = dict["leed_id"] as? String
            }else{
                cell.detailTextLabel?.text = "Not available"
            }
            return cell
            ...
            if(dict["created_at"] != nil && (dict["created_at"] as? String)?.characters.count > 0){
                cell.detailTextLabel?.text = dict["created_at"] as? String
            }else{
                cell.detailTextLabel?.text = "Not available"
            }
            return cell
